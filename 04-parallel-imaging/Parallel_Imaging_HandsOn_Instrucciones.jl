...
begin
	Dict_m = matread("C:\\Users\\ronal\\Desktop\\Taller_Suckerber\\Mk_Parallel_Imaging.mat")
	M_kj = Dict_m["Mk"]
	m_xj = complex(zeros(size(M_kj)))
	#local m_x_sos = zeros(size(M_kj,2),size(M_kj,3))
	for i in 1:size(M_kj,1)	
		m_xj[i,:,:] =  ifftshift(ifft( ifftshift(M_kj[i,:,:]) ),2)
		#m_x_sos = m_x_sos + conj.(m_xj[i,:,:]).*m_xj[i,:,:] 
	end
	
	
	heatmap([abs.(m_xj[1,:,:]) abs.(m_xj[2,:,:]); abs.(m_xj[3,:,:]) abs.(m_xj[4,:,:])], c=:grays)
end

# ╔═╡ e81a994b-8c5b-4423-a30b-337cf6b9e06f
begin
...
# ╔═╡ 2b5a4037-33e1-4eb5-ba85-c92ef3599e4c
begin
	#Diseñe el operador E y E_h
...
###### Ejercicio: Construya una solucion al problema $E^{H}Em(x)=E^{H}M(k)$ usando gradiente conjugado para conseguir el $m(x)$. Consiga la solución para factores de sumuestro R = 2 y R = 4, grafiquelas en conjunto con el ground
"""

# ╔═╡ bf301887-6144-454b-9d65-bc4bda3f284d
...
# ╔═╡ cf2e8000-6181-40e0-b0c1-4c0ffcd59fc7
begin
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "ddfa8abc91e8a354a7d4d9af5e6707f07f8e2687"

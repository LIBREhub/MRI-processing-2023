...
md"""
# Reconstrucción Submuestreada con Compressed Sensing: hands-on
"""

# ╔═╡ 9919d2ec-0803-4337-9143-0e89ccd05f41
md"""
!!! alert "Objetivo de trabajo"
	El objetivo de este trabajo es reconstruir datos de resonancia magnética de diferentes tipos:

	- Reconstrucción de datos ralos adquiridos con un factor de aceleración 6 con patrón de submuestreo variable.

	- Observación de la reconstrucción de datos ralos adquiridos con un factor de aceleración 6 con patrón de submuestreo uniforme.

	- Reconstrucción de datos no ralos adquiridos con un factor de aceleración 6 con patrón de submuestreo variable.
"""
...
md"""
!!! note "Información para trabajar"
	En la carpeta *data* encontrará los archivos necesarios para trabajar:
	* **raw_data.mat** contiene la información de una adquisición submustreada con factor de aceleración 6. Esta es una matriz de 42 filas y 256 columnas. Considere que los datos completamente muestreados deberían ser una matriz de 256 filas y 256 columnas.
	* **Uv.mat** contiene la inframción del patrón de submuestreo con el que fueron adquiridos los datos. Este es una matriz de 256 filas y 256 columnas que contiene filas que valen 0 (filas que no fueron muestreadas) y filas que valen 1 (filas que fueron completamente muestreadas).
"""
...
md"""
!!! note "Solución Ingenua"
	A continuación observaremos las reconstrucciones con *aliassing* obtenidas de reconstruir directamente los datos submuestreados. Analicemos la importancia de posicionar la *raw data* en las líneas que efectivamente corresponden al muestreo.
"""
...
begin
	heatmap(abs.(ifft(raw_data)), color=:grays, c=:grays)
...
begin
	...
	for i in 1:255
		if sum(Uv[i,:])!=0
			contador+=1;
			M_COMPLETE[i,:].=raw_data[contador,:]
		end
	end
...
md"""
!!! note "Reconstrucción con Compressed Sensing"
	Para resolver el problema de reconstrucción submuestreada con compressed sensing vamos a suponer que la señal submuestreada es **rala**, esto significa que tiene una norma $$\ell^0$$ pequeña. Esto nos permite plantear el siguiente problema de optimización:

	$$\min_{\boldsymbol{x}}\|\boldsymbol{x}\|_0$$
	$$\text{s.t} \|A\boldsymbol{x}-\boldsymbol{b}\|_2<\varepsilon$$
	La minimización de la norma $\ell^0$ no es un problema convexo, por lo que para resolver el problema de optimización se sustituye la norma $\ell^0$ por su relajación convexa que es la norma $\ell^1$. De esta forma el problema de optimización se expresa de la forma:

	$$\min_{\boldsymbol{x}}\|\boldsymbol{x}\|_1$$
	$$\text{s.t} \|A\boldsymbol{x}-\boldsymbol{b}\|_2<\varepsilon$$
	Si ahora realizamos la relajación lagrangiana del problema este se puede expresar como:

	$$\min_{\boldsymbol{x}}\|A\boldsymbol{x}-\boldsymbol{b}\|_2+\lambda \|\boldsymbol{x}\|_1$$
	...
	* Uv es el patrón de submuestreo
	* k_subsamp es el espacio k submuestreado
	* lambda es el parámetro de regularización
	* iters es el número máximo de iteraciones
	* start es el punto de partida
"""

# ╔═╡ 7305f393-d980-4d96-ac8a-c8b7a7b97313
begin

function FISTA_L1(M_C, b, lambda, iters, start)    
    L = 1
...
begin
	M_FISTA = FISTA_L1(Uv, M_COMPLETE, 0.5, 100, ones(size(M_COMPLETE)));
...
!!! note "Información para trabajar"
	En la carpeta *data* encontrará los archivos necesarios para trabajar:
	* **raw\_data\_2.mat** contiene la información de una adquisición submustreada con factor de aceleración 6. Esta es una matriz de 42 filas y 256 columnas. Considere que los datos completamente muestreados deberían ser una matriz de 256 filas y 256 columnas.
	* **Ut.mat** contiene la inframción del patrón de submuestreo con el que fueron adquiridos los datos. Este es una matriz de 256 filas y 256 columnas que contiene filas que valen 0 (filas que no fueron muestreadas) y filas que valen 1 (filas que fueron completamente muestreadas).
"""
...
begin
	heatmap(abs.(ifft(raw_data_2)), color=:grays, c=:grays)
...
begin
	...
	for i in 1:255
		if sum(Ut[256-i,:])!=0
			contador_2+=1;
			M_COMPLETE_2[i,:].=raw_data_2[contador_2,:]
		end
	end
...
begin
	M_FISTA_2 = FISTA_L1(Ut, M_COMPLETE_2, 0.5, 100, ones(size(M_COMPLETE_2)));
...
!!! note "Información para trabajar"
	En la carpeta *data* encontrará los archivos necesarios para trabajar:
	* **raw\_data\_3.mat** contiene la información de una adquisición submustreada con factor de aceleración 6. Esta es una matriz de 42 filas y 256 columnas. Considere que los datos completamente muestreados deberían ser una matriz de 256 filas y 256 columnas.
	* **Uv.mat** contiene la inframción del patrón de submuestreo con el que fueron adquiridos los datos. Este es una matriz de 256 filas y 256 columnas que contiene filas que valen 0 (filas que no fueron muestreadas) y filas que valen 1 (filas que fueron completamente muestreadas). Este ya fue importado en el ejercicio 1 por lo que no es necesario volder a importarlo.
"""
...
begin
	heatmap(abs.(ifft(raw_data_3)), color=:grays, c=:grays)
	title!("Reconstrucción de raw data con IDFT")
end
...
begin
	...
	for i in 1:255
		if sum(Uv[i,:])!=0
			contador_3+=1;
			M_COMPLETE_3[i,:].=raw_data_3[contador_3,:]
		end
	end
...
md"""
!!! note "Reconstrucción con Compressed Sensing"
	Ya que la imagen submuestreada no es rala, no podemos aplicar directamente el problema de optimización de Compressed Sensing, porque en este caso estamos buscando un $\boldsymbol{x}$ cuya norma $\ell^0$ no es pequeña, por lo que no tiene sentido minimizarla. En este caso, buscamos un dominio en el que la imagen sí sea rala para poder realizar la optimización y ahí reconstruir. En este caso utilizaremos el dominio de wavelet.
"""
...
title!("Transformada de Wavelet imagen co aliassing")
end
...
begin
M_FISTA_3 = FISTA_WAVELET(Uv, xt, 100, 5, ones(size(M_COMPLETE)));

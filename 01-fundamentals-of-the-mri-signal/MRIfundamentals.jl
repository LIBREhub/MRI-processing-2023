...
# ╔═╡ defa990e-857f-11ee-1912-63afa1085dc9
# Import modules
using FFTW, Plots, Images

# ╔═╡ 197341cc-e9a0-4b4b-95e9-47901859e5e0
md"""
# Fundamentos de la señal de MRI

1. Simula el objeto continuo en el dominio de Fourier
2. Define una estrategia de muestreo (trayectoria en espacio k)
3. Muestrea el dominio continuo
4. Reconstruye la imagen
"""

# ╔═╡ ce7414b2-0b29-4cac-96be-e970b52aa0fd
md"""
## 1. Simula dominio continuo
* Define el objeto, como suma de rectángulos:
$m(x,y) = \sum_i \sqcap\left(\frac{x-c_{xi}}{W_{xi}}\right) \sqcap\left(\frac{y-c_{yi}}{W_{yi}}\right)$
* En el dominio de Fourier, esto es
$M(k_x,k_y) = \sum_i \mbox{sinc}\left(W_{xi}k_x\right) \mbox{sinc}\left(W_{yi}k_y\right) e^{-i2\pi (c_{xi} k_x + c_{yi} k_y) }$
* donde
$\mbox{sinc}(x) = \frac{\sin(\pi x)}{\pi x}$
* These formulas will define a function that can be sampled
$M_\mbox{cont}(k_x,k_y)$

"""

# ╔═╡ b2af8091-5949-4a19-aea9-64ce982f9c10
# Define the sinc function
function sinc(x)
	return x == 0 ? 1.0 : sin(π * x) / (π * x)
end

# ╔═╡ 3e18be43-0048-4314-9bbb-2013b74a51c2
# Define one square in k-space domain
function make_square(kx, ky, width, center)
	return (
		(width[1] * sinc.(width[1] * kx))
		.* (width[2] * sinc.(width[2] * ky))
		.* exp(-1.0im * 2 * π * ((center[1] * kx) .+ (center[2] * ky)))
	)
end

# ╔═╡ f655679c-050f-4cf6-bea1-d932921b806f
begin
	Widths = [12 16; 5 10]; 	# sizes of squares in cm (one square per row)
	Centers = [0 0; 2 6];    # centers of squares in cm
	nothing
end

# ╔═╡ 7c660539-b97e-4a43-9fc4-76d4cf05cba2
# Define a function that adds two squares in k-space to generate M(k_x,k_y)
function Mcont(kx, ky)
    M1 = make_square(kx, ky, Widths[1,:], Centers[1,:])
    M2 = make_square(kx, ky, Widths[2,:], Centers[2,:])
    return M1 + M2
end

# ╔═╡ 6f8bbfd7-c694-4785-bfa2-16f378f400fc
md"""
...
# ╔═╡ 5237b5de-1dfa-4215-951c-00f2619b469d
# Plot the sampled k-space domain
begin
	kxlim, kylim = maximum(abs.(kx)), maximum(abs.(ky))
	klim = 1.1 * maximum([kxlim, kylim])
	plot(kx, ky,
		xlim=[-klim, klim], ylim=[-klim, klim],
		aspect_ratio=:equal, seriestype=:scatter, markersize=0.2, legend=false,
		title="Sampled k-space domain",
		xlabel="kx [1/cm]",  ylabel="ky [1/cm]"
	)
end

# ╔═╡ 9f0e33c1-4b72-4992-99e5-7a5bcffa9cd5
md"""
## 3. Muestrea el dominio continuo

"""

# ╔═╡ 3cbe2dab-2991-462c-b46c-3c96214b6641
# Sampled continuous object in k-space domain
M = Nx * Ny * Mcont.(kx, ky) / (FOVx * FOVy);

# ╔═╡ a1b54d78-6418-4158-a266-f55ccd761abc
begin
	# Plot the logaritmic magnitude of the k-space
	heatmap(kxline, kyline, log.(abs.(M) .+ 1),
		aspect_ratio=:equal, color=:grays,
		title="Sampled square object in the K-space domain"
	)
end
...
md"""
## 4. Reconstrucción
Emplea directamente la FFT. Nótese que la FFT define el origen en una esquina, por lo que es necesario trasladar nuestros datos a la esquina, fftshift(M), y luego, después de la FFT es necesario volverlos al centro con otro comando fftshift().

"""

# ╔═╡ 411c3176-4915-4626-a87f-8b0a7a639f9f
# Reconstrucción directa con la FFT
begin
	mrec = fftshift(ifft(fftshift(M)))
	size(mrec)
end

# ╔═╡ 5baab0ab-37da-4cb8-8d9c-a0e87e76b101
md"""
El resultado de la reconstrucción es una matriz de Ny x Nx. El tamaño de cada pixel es FOVx/Nx en el sentido x, y FOVy/Ny en el sentido y. Para mostrar esta imagen con su tamaño real es necesario asegurarse que cada pixel sea del mismo tamaño.
...
# Plot de la reconstrucción
heatmap(xlinetrue, ylinetrue, abs.(mrectrue),
#heatmap(abs.(mrectrue),
	aspect_ratio=:equal, color=:grays,
	title="Reconstrucción con proporciones correctas"
)

# ╔═╡ ff4b6756-c7eb-4ef3-8e15-2c56bbe377a6
# Defines trajectory with reduced FOV
#FOVx, FOVy= 25, 18; 	# FOV in cm
# Sampling matrix size (low resolution)
#Nx, Ny = 32, 64; # number of pixels
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "823cfad37f482ae0fc20d8546b7741534b40e799"
